...
//-------------------------------------------------------------------------------
    ...
    for (uint8_t h=0; h<4; h++){
      bitSet(ADCSRA,ADSC); while(bit_is_set(ADCSRA,ADSC));  
      // ADC readings take 200microseconds each while waiting for the coincell voltage drop to occur
    }

    uint16_Buffer=ADC; //ADC is a macro command that combines the ADC's two output memory registers into one integer
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  Serial.flush();
  EEmemoryPointr = 64;                         // reset back to startup default for the main loop
