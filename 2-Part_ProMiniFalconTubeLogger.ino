...
---------------------------------------------------------------------------
  BH1750 LUX sensor uses hp_BH1750  [by Stefan Armborst] via Lib Manager OR from https://github.com/Starmbi/hp_BH1750
...
// AM312 Infrared Motion sensor:           // typically adds 10-15uA to sleep current //PIR drives its output line HIGH when it detects motion
//#define PIRcountPerInterval_2byte        // 2-bytes:  PIR is just an interrupt generating sensor, saves # of PIR HIGH events in a specified sample interval. Do not enable this with PIRtriggersReadings_4byte - choose one or the other
...
// PIR readme:
// PIRtriggers DOES NOT use the regular RTC-alarm based sampling interval but instead records the seconds elapsed between EVERY PIR trigger event in a uint32_t long variable [NOTE uint16_t overflows if no triggers within ~18 hours so change variable if you want longer intervals]
...
// AM312 has a hold time of about 2.0-2.3 sec and a blocking (cooldown) time of about 2 seconds, sensor STAYS HIGH indefinately if a new triggers occur when it is already high
...
// TTP223 Capacitive Touch Switch:        // can be configured with jumpers for simmilar behavior to the AM312 - although the PIR has much longer (2s) signal HOLD-UP time
//#define TTP223countPerInterval_2byte    // 2-bytes: TTP223 is another interrupt generating sensor, saves #of Touch switch HIGH events in the start menu specified sample interval. Do not enable this WITH TTP233 triggersReadings_4byte - choose one mode  or the other
//#define TTP223triggersReadings_4byte    // Do not enable this with TTP223countPerInterval_2byte - choose ONE OR THE OTHER: saves 4-bytes Unix Timestamp every record so uses memory quickly
// TTP223 readme:                         // Like the PIR, TTP223triggers DOES NOT use the regular RTC-alarm sampling interval
// "Momentary" is typically the default for these modules with BOTH A AND B terminals left OPEN. For config info see: https://diyusthad.com/2019/06/how-to-use-ttp223-based-touch-switch.html
// With A&B open the output signal changes from (LOW) to state (HIGH) when a touch is detected and should return to (LOW) state when released
// SECURE THE TOUCH sensor WITH DROPS OF HOT GLUE on the side to avoid shaking the fragile breadboard connections!
// ALSO: trim down the header pins poking above the sensor surface board and cover them with conformal - if your finger touches the metal pins the touch will not register
// It may help to solder a small bypass cap directly onto the sensor module to stabilize power connections
// SLOW SENSOR RESPONSE TIME: ~220ms at VDD = 3V, Power-On Stabilization takes 0.5 seconds - Do not touch the pad during startup.
// The TTP223/TTP233 performs periodic auto-calibration. If environmental capacitance shifts during touch, the internal timer recalibrates the baseline after roughly 4 seconds.
// TTP233s do this after EVERY touch. If your finger is on the pad during the recalibration it may cause weird behavior or invert the response!
// Capactive sensors are sensitive to nearby interference: see https://forum.arduino.cc/t/ttp223-touch-sensors-self-triggering-bug/414967
...
#include <LowPower.h>   // this library NEEDS TO BE INSTALLED via the LIBRARY MANAGER for interval & battery recovery sleeps
                        // OR can be downloaded from LowPowerLab's repo (https://github.com/LowPowerLab/LowPower)
...
#define totalBytesOfStorage 32768        // Default: 0x57 / 4096 bytes for 4k // 32k Module: 0x50 & 32768 
...
int16_t rtc_TEMP_degCx4 = 0;                  // NOTE saved rtc_TEMP is reduced to only one BYTE
...
#if defined(PIRcountPerInterval_2byte) || defined(PIRtriggersReadings_4byte) || defined(TTP223countPerInterval_2byte) || defined(TTP223triggersReadings_4byte)
uint16_t d3_INT1_eventCounter = 0;      // only saved with countPerInterval defines
volatile boolean d3_INT1_Flag = false;  // needed for dual interrupt combinations
#endif

#if defined(PIRtriggersReadings_4byte) || defined(TTP223triggersReadings_4byte)
//----------------------------------------------------------------------------
...
#ifdef readD7resistorwD6ref_2byte 
//--------------------------------
  uint32_t D7resistor_NewReading;           // max of 65535 limits our ability to measure 10kNTC at temps below zero C!
  // although the timing code is capped at 65535 the variable needs to be uint32_t to accomodate
  // because clock ticks are about 1/3 of actual ohms & calculation can reach values of about 150k ohms 
#endif

#ifdef readD9resistorwD6ref_2byte 
//--------------------------------
  uint32_t D9resistor_NewReading;           // 65535 limit
#endif
#if defined(readD7resistorwD6ref_2byte) || defined(readD9resistorwD6ref_2byte)
//------------------------------------------------
  #define referenceResistorValue 10000UL    //  ARBITRARY value that should be 'close' to actual ohms but precise is not required  //https://hackingmajenkoblog.wordpress.com/2016/08/12/measuring-arduino-internal-pull-up-resistors/
...
// The SHT85 is protocol in this library is compatible with the SHT3x series of sensors
  ...
  float Sht3x_Temp_degC,Sht3x_RH_percent;   // floats converted to integer before saving - preserving two decimal places
...
//-----------------------------------------------------------------------------------
  ...
  // NOTE these floats get converted into 16-bit ints before saving to EEprom
#endif
...
//-----------------------------------------------------------------------------------
  ...
  int32_t Bmx_temperature;  // current temperature with 2 decimal places ALREADY embedded in the integer by the library
  uint32_t Bmx_pressure;    // current pressure           "
  uint32_t Bme_humidity;    // current humidity           "
#endif
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  #if defined(PIRcountPerInterval_2byte) || defined(TTP223countPerInterval_2byte)
...
//------------------------------------------------------------------------------------------------------------
  ...
  #else
    for (int i = 9; i <=12; i++) { pinMode(i,INPUT); digitalWrite(13,HIGH);}
  #endif
...
//------------------------------------------------------------------------------------------------------------
  ...
  ADMUX = default_ADMUX; ADCSRA = default_ADCSRA;   //restore the default
...
//------------------------------------------------------------------------------------------------------------
  ...
  // later on the in the main looploop
...
do { command = Serial.readStringUntil('\n');                  // read serial monitor data into the string until carridge return = \n character
    ...
    if(command == "start"){ 
      ...
      for (uint32_t memoryLocation=0; memoryLocation<totalBytesOfStorage; memoryLocation+=16){  // loop writes 16-bytes at a time into the I2C buffer
          ...
          for (byte k=0; k<16; k++) { Wire.write(0xFF);}         // we are loading the I2C buffer with 16 'zeros' each time
...
// no initialization needed for readD7resistorwD6ref_2byte or readD9resistorwD6ref_2byte
...
//------------------------------------------------------------------------------  
  Serial.println(F("Bl&Gr LEDs 'flicker' for SyncDelay til 1stRead, then GREEN pips @sample time"));  
...
//----------------------------------------------------------------------------------
turnOffAllindicatorLEDs();  // draws about 50uA per LED channel with internal pullup resistors
...
//-------------------------------------------------------------------------------    
...
#endif //terminates #if defined(PIRtriggersReadings_4byte) || defined(TTP223triggersReadings_4byte)    
 
  if(ECHO_TO_SERIAL){
      Serial.println();Serial.println();
        #if defined(PIRtriggersReadings_4byte) || defined(TTP223triggersReadings_4byte)
          Serial.print(F(">D3 interrupt wake#")); Serial.print(d3_INT1_eventCounter);Serial.print(F(": "));
        #else
          Serial.print(F(">Wake: "));
        #endif
  ...
  #ifdef logRTC_Temperature_1byte
    rtc_TEMP_degCx4 = RTC_DS3231_getTemp();
      if(ECHO_TO_SERIAL){
        Serial.print(F("RTC[°C]: "));floatBuffer=rtc_TEMP_degCx4/4.0;Serial.print(floatBuffer,2);
      } 
  #endif
...
//--------END of RTC activity-----------------
    ...
    freeVariableMemory = freeRam();                   
    if(ECHO_TO_SERIAL){
        Serial.print(F(", VarMem(bytes):"));Serial.print(freeVariableMemory);Serial.flush();
        }
#endif

#ifdef logLowestBattery_1byte
      if(ECHO_TO_SERIAL){ 
      Serial.print(F(", LowBat[mV]:"));Serial.print(LowestBattery);Serial.flush();
      }
 #endif
...
#if defined(readD9resistorwD6ref_2byte) || defined(readD7resistorwD6ref_2byte)
//------------------------------------------------------
...
#ifdef readD9resistorwD6ref_2byte   
//-------------------------------
  //alternate wiring: D9resistor_NewReading = ReadD6riseTimeOnD8();
...
//-------------------
   ...
   LowPower.powerDown(SLEEP_30MS, ADC_OFF, BOD_OFF);;//At lowest resoluton this should be plenty of time 
    ...
    Bmx_temperature = climateSensor.getTemperatureCelsius();
    if(ECHO_TO_SERIAL){Serial.print(F(" BmeT: "));Serial.print(Bmx_temperature/100);Serial.print(F("."));Serial.print(Bmx_temperature%100);Serial.print(F("°C"));}
   #endif
   #if defined(recordBMEpressure_2byteInt)
    Bmx_pressure = climateSensor.getPressure();
    if(ECHO_TO_SERIAL){Serial.print(F(" BmePr: "));Serial.print(Bmx_pressure/100);Serial.print(F("."));Serial.print(Bmx_pressure%100);Serial.print(F("hPa"));}
   #endif
   #if defined(recordBMEhumidity_2byteInt)
    Bme_humidity = climateSensor.getRelativeHumidity();
    if(ECHO_TO_SERIAL){Serial.print(F(" BmeRh: "));Serial.print(Bme_humidity/100);Serial.print(F("."));Serial.print(Bme_humidity%100);Serial.print(F("%"));}
   #endif
    if(ECHO_TO_SERIAL){Serial.println();Serial.flush();}
#endif
...
#endif
#ifdef TTP223countPerInterval_2byte
    if(ECHO_TO_SERIAL){ Serial.print(F(" ,TTP233 total:")); Serial.print(d3_INT1_eventCounter);Serial.flush();}
#endif
//-----------------------------------------------------
...
//bitWrite(ADCSRA,ADPS2,1);bitWrite(ADCSRA,ADPS1,1);bitWrite(ADCSRA,ADPS0,0);   // 64 (default) prescalar @ 8MHz/64 = 125 kHz, =~104uS/ADC reading
  ...
  // NOTE: Aref capacitor Rise time can take 5milliseconds after starting ADC so 15ms of ADC_ON powerDown sleep works 
...
//---------------------------------------------------------------------------------
  ...
  Wire.setClock(100000UL); // Set I2C bus speed to 100 kHz
  //when working with the default 4K eeprom on the RTC module
...
//-------------------------------------------------------------------------------------------------------
  ...
  if(byteBuffer1==255){byteBuffer1=254;}          // Trapping 0xFF to preserve End of Data check in startMenu_sendOPDreadings2Serial
...
//--------------------------------------------------------------------------------------------------------
  int16_Buffer = rtc_TEMP_degCx4 + RTC_TempRangeOffset; // add 8*10 shifts the 4xInt 63 degree range by 20C, so we can record -20C to +43C with no loss of information because the resolution is only 0.25C              
  if(int16_Buffer>254){int16_Buffer=254;}         // ONLY THE FIRST data byte saved in each record must have a ZERO TRAP to preserve zero EOF indicators in EEprom
  if(int16_Buffer<0){int16_Buffer=0;}
  byteBuffer1 = lowByte(int16_Buffer);
...
//---------------------------------
// D7resistor_NewReading changed to 4-byte uint32 to avoid calculation overflows
// but we only record the lowest two of those bytes here
// this requires bitmath extraction of high & low bytes for eeprom storage
  uint32_Buffer = D7resistor_NewReading; 
    if (uint32_Buffer>65534){uint32_Buffer=65534;}    // Prevents uint16_t Overflow & Traps 0xFF
...
// this requires bitmath extraction of high & low bytes for eeprom storage
  uint32_Buffer = D6resistor_NewReading; 
    if (uint32_Buffer>65534){uint32_Buffer=65534;}    // Prevents uint16_t Overflow  // & Trapping 0xFF
  byteBuffer1   = uint32_Buffer & 0b11111111; //lobyte
    Wire.write(byteBuffer1); 
  uint32_Buffer = uint32_Buffer>>8;
  byteBuffer2   = uint32_Buffer & 0b11111111; //highByte
...
#ifdef readBh1750_LUX_2byte
//-------------------------
  if (lux_BH1750_RawInt==65535){lux_BH1750_RawInt=65534;}  // Trap 0xFF to preserve EOF check in startMenu_sendOPDreadings2Serial
  loByte = lowByte(lux_BH1750_RawInt);
        Wire.write(loByte);
...
#ifdef recordBMEtemp_2byteInt            // stored in int32_t Bmx_temperature with 2 decimals 'embedded' in the integer
//---------------------------
  int16_Buffer = Bmx_temperature;        // room in int16 to ±32,000 so no danger of overflow
...
#ifdef recordBMEpressure_2byteInt        // stored in int32_t Bmx_pressure with 2 decimals 'embedded' in the integer
//-------------------------------
  uint16_Buffer = Bmx_pressure - 80000;  // -80000 for SCALING to fit into uint16 with two preserved decimal places
  if(uint16_Buffer==65535){uint16_Buffer=65534;} // Traps 0xFF to preserve EOF check in startMenu_sendOPDreadings2Seriall
...
#ifdef recordBMEhumidity_2byteInt       // stored in int32_t Bme_humidity with 3 decimals 'embedded' in the integer
//-------------------------------
  uint16_Buffer = Bme_humidity;
  if(uint16_Buffer==65535){uint16_Buffer=65534;}
  loByte = lowByte(uint16_Buffer); 
      Wire.write(loByte);
  hiByte = highByte(uint16_Buffer);
      Wire.write(hiByte);
...
//-----------------------------------------------------------------------------------------------------------------------
  uint16_Buffer = CurrentBattery;
  if(uint16_Buffer==65535){uint16_Buffer=65534;}
  loByte = lowByte(uint16_Buffer);           // note: we save the low byte first because it is almost never zero
...
//-----------------------------------------------------------------------------------------------------------------------
  uint16_Buffer = freeVariableMemory;
  if(uint16_Buffer==65535){uint16_Buffer=65534;}
  loByte = lowByte(uint16_Buffer);
        Wire.write(loByte);              
  hiByte = highByte(uint16_Buffer);
...
//-----------------------------------------------------------------------------------------------------------------------
  uint16_Buffer = TEMP_si7051;
  if(uint16_Buffer==65535){uint16_Buffer=65534;}
  loByte = lowByte(uint16_Buffer);              //NOTE TEMP_si7051 overruns uint16_t if >40C!
        Wire.write(loByte);
  hiByte = highByte(uint16_Buffer); 
        Wire.write(hiByte);
...
//-------------------------------------------------------------------------------
    ...
    ADMUX = default_ADMUX; ADCSRA = default_ADCSRA;
...
if(totalBytesOfStorage>4096){
    //larger eeproms get a period of IDLE before going to sleep
    //but IDLE is not compatible with the 4k
...
// AND make sure you don't over-run your alarm interval!
  ...
  floatBuffer=rtc_TEMP_degCx4/4.0;oled.print(floatBuffer,2);
  ...
  #ifdef readD9resistorwD6ref_2byte
  oled.setCursor(32,6);oled.print(F("D9r"));
  oled.setCursor(56,7);oled.print(D9resistor_NewReading);
  #endif //ReadLDR
...
// NOTE: nesting here is somewhat complex with the different D3 interrupt response modes:

#if defined(PIRcountPerInterval_2byte) || defined(TTP223countPerInterval_2byte) 
    // This is a 'processor trapping loop' for interrupt counting that is independant of the main loop! // Logger can be woken from this trap by D2 AND D3 interrupt events

    rtc_INT0_Flag = false; // clear the flag we use to indicate the RTC alarm occurred before entering the 'counting trap'

    do{ // this do-while locks the processor into a 'D3 counting loop' until the RTC alarm happens and its flag is set true

      if(ECHO_TO_SERIAL && rtc_INT0_Flag==false && d3_INT1_eventCounter>0) {Serial.println();Serial.print(F(" D3int#")); Serial.print(d3_INT1_eventCounter);Serial.flush();}

      // AM312 PIR has a HOLD-UP time of ~2 seconds and a blocking (cooldown) time of about 2 seconds before it can trigger again
      // proceed to sleepNwait only AFTER the sensor has reset to LOW, NOTE: PIR stays HIGH if new trigger happens before reset
      #if defined(PIRcountPerInterval_2byte) 
        noInterrupts();bitSet(EIFR,INTF1); //pinMode(3,INPUT);
        attachInterrupt(1,input_d3_interrupt_ISR,LOW); interrupts(); turnOnGreenLED();
        while(digitalRead(3) == HIGH) { LowPower.powerDown(SLEEP_500MS, ADC_OFF, BOD_OFF);}
        noInterrupts();detachInterrupt(1);bitSet(EIFR,INTF1);interrupts(); turnOffAllindicatorLEDs();
        LowPower.powerDown(SLEEP_15MS, ADC_OFF, BOD_OFF);  // 15ms settling time for the interrupt line capacitance
      #endif  //defined(PIRcountPerInterval_2byte)
    
      sleepNwait4D3InterruptORrtcAlarm();             // d3_INT1_eventCounter gets incremented inside the sleepNwait4D3Interrupt() function

    }while (rtc_INT0_Flag == false);                  // if RTC alarm fires Flag becomes true in the ISR & we break out of the PIR do-while counting loop

#else
    
      #if defined(PIRtriggersReadings_4byte)
        noInterrupts();pinMode(3,INPUT);bitSet(EIFR,INTF1);
        attachInterrupt(1,input_d3_interrupt_ISR,LOW);
        interrupts(); turnOnGreenLED();
        while(digitalRead(3) == HIGH) {LowPower.powerDown(SLEEP_500MS, ADC_OFF, BOD_OFF);}
        noInterrupts();detachInterrupt(1);bitSet(EIFR,INTF1);interrupts();turnOffAllindicatorLEDs(); 
        LowPower.powerDown(SLEEP_15MS, ADC_OFF, BOD_OFF);  // 15ms settling time for the interrupt line capacitance
      #endif
  ...
  #if defined(PIRtriggersReadings_4byte) || defined(TTP223triggersReadings_4byte)
          sleepNwait4D3InterruptORrtcAlarm();  // in this case the logger wakes ONLY by D3 interrupt going HIGH - because RTC alarm was disabled in setup
    #else
          sleepNwait4RTCalarm();      // NORMAL RTC-only sleepNwait with no D3 interrupt-event-counters
  #endif
    
#endif //terminates #if defined(PIRcountPerInterval_2byte) || defined(TTP223countPerInterval_2byte)
...
void sleepNwait4RTCalarm() {                          //NOTE all existing pin states are preserved during sleep
  ...
  // FLOAT all pins changed nothing on high 100ua sleepers?
  ...
  //HERE AFTER WAKING  // note that detachInterrupt(0); happens inside the ISR
...
void sleepNwait4D3InterruptORrtcAlarm(){
  ...
  // PIR drives its output line HIGH when it detects motion
  // TTP223 Touch Switch is the same when both A and B terminals should be in open position
  // But TTP223 behavior can INVERT if accidentally touched during self-calibration
...
#endif  //terminates #if defined(PIRcountPerInterval_2byte) || defined(PIRtriggersReadings_4byte) || defined(TTP223countPerInterval_2byte) || defined(TTP223triggersReadings_4byte)
...
//  first 0-63 =64 bytes reserved for logger parameters & constants
  ...
  Serial.print(F(" @")); Serial.println((__FlashStringHelper*)compileTime);
    ...
    // The ASCII code for a blank space is decimal 32 which disables the output
    // unformatted eeprom usually contains 255 which prints as ÿ (y with a diaeresis) this also surpresses output
    byteBuffer1 = EEPROM.read(266);
    if(byteBuffer1!=255 && byteBuffer1!=32){ // checks prevent serial output if text field is empty    Serial.println();
    ...
    byteBuffer1 = EEPROM.read(367);  // unformatted eeprom usually contains 255 which prints as ÿ (y with a diaeresis)
    if(byteBuffer1!=255 && byteBuffer1!=32){             // check prevents serial display if text field is still empty
    Serial.println();   
...
// note: setup_sendboilerplate2serialMonitor(); runs once on startup before this
  ...
  Serial.print(F("RUNtime: "));Serial.print(totalBytesOfStorage);
  Serial.print(F(" / "));Serial.print(sensorBytesPerRecord);Serial.print(F("recBytes @ "));
    ...
    Serial.print(F(" ~ "));
    floatBuffer = ((totalBytesOfStorage)/sensorBytesPerRecord);
    ...
    Serial.println(F(" [4]  Deployment info   [5]  SERIAL On / Off  [6]  START logging"));
...
// because these are the data collumn headers used in startMenu_sendData2Serial
        ...
        Serial.print(F("PIRtrigger = Variable Runtime!"));
        #endif
  #ifdef TTP223triggersReadings_4byte
        Serial.print(F("TTP223trigger = Variable Runtime!"));
        #endif  
  }
...
//starting time value was stored in first four bytes of the 328p internal eeprom:
  ...
  EEmemPointer = 0;                                     // a counter that advances through the EEprom Memory in sensorBytesPerRecord increments
  byte checkingEOF = 255;     

  do{    // this big do-while loop readback must EXACTLY MATCH the data saving pattern in our main loop:
  //---------------------------------------------------------------------------------------------------------- 
  
  checkingEOF = 255; byteBuffer2 = 255;
  ...
  if((EEmemPointer+2) < totalBytesOfStorage){
    checkingEOF = i2c_eeprom_read_byte(EEpromI2Caddr,EEmemPointer+2);
    }
  if(convertDataFlag && byteBuffer1==255 && byteBuffer2==255 && checkingEOF==255){ break;}     
  // multiple 0xFF breaks us out of the do-while readback loop, recall that EEprom was ERASED by setting all locations to 0xFF at logger startup
  // this EndOfFile check only happens if convertDataFlag true, otherwise the data dump runs through the entire eeprom
  // Checks for THREE SUCCESSIVE 0xFF as the end of data marker in the eeprom: checking multiple is less dependant on 0xFF traps & handles one spurious readback fail
...
#ifdef logRTC_Temperature_1byte                                   // RTC temperature 1-byte compressed: low side cutoff depends on range offset used
      uint16_Buffer = i2c_eeprom_read_byte(EEpromI2Caddr,EEmemPointer);EEmemPointer++;            
      int16_Buffer = uint16_Buffer - RTC_TempRangeOffset;   // reverses the + Offset shift to handle more negative temps in our 0-255 byte
      int16_Buffer = int16_Buffer*25;                       // combines *100 AND /4 to restore 0.25, 0.5, 0.75 fractional digits
      floatBuffer = int16_Buffer/100.0;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#ifdef recordBMEhumidity_2byteInt // originally uint32_t Bme_humidity but we truncated to 2-bytes
      loByte = i2c_eeprom_read_byte(EEpromI2Caddr,EEmemPointer);
      EEmemPointer++;
      hiByte = i2c_eeprom_read_byte(EEpromI2Caddr,EEmemPointer);
      EEmemPointer++;
  ...
  Serial.flush();
  EEmemPointer = 0; // reset back to startup default for the main loop
...
uint16_t readRailVoltage(){                             // reads 1.1vref as input against VCC as reference voltage
  ...
  ADMUX = default_ADMUX; ADCSRA = default_ADCSRA;       // restores default A3 channel
...
void error_shutdown() {
  ...
  // when an alarm is triggered, the INT/SQW pin goes low, and it stays that way until the appropriate register is cleared in the DS3231
  // this results in ~700uA drain through the pullup AFTER shutdown which depletes the coincell battery
  // STOPPING THE RTC OSCILATOR is another way to prevent those eventual alarms from pulling SQW low:
...
int16_t RTC_DS3231_getTemp(){  // Returns int16_t temperature in Celsius times four.
//--------------------------------------------------------------------------------------------
// temp registers (upper byte 0x11 & lower 0x12) get updated automatically every 64s

    union int16_byte {                  //here a union is used to merge the two bytes
        int16_t i;
        uint8_t b[2];
    } rtcTemp;
  ...
  if (Wire.available()) {
    ...
    rtcTemp.b[0] = Wire.read();         // loByte = fraction portion
    return rtcTemp.i /64;               // cant use bit shifting because signed
    }  
    else { return (255-RTC_TempRangeOffset);}  
    // the max our encoding can store to flag there has been a read error
}
...
// ============================================================================================================
// SI7051 TEMPERATURE SENSOR - here as an 'example' but we will may use this particular sensor in the course
// ============================================================================================================
...
// FAST PORT COMMANDS used throughout these functions to increase timing precision
...
//========== read resistor RISING on D6 ===========
  ...
  noInterrupts(); sleep_enable(); TCNT1 = 0; // reset Timer 1 counter value to 0
  bitSet(PORTD,6);                        // D6 HIGH -> now charging the cap through 10k ref
  do{
    interrupts(); sleep_cpu(); noInterrupts();  // note: sleep_disable()happens inside ISR (TIMER1_CAPT_vect)
    ...
    bitSet(DDRB,0);//bitClear(PORTB,0);   // D8 OUTPUT & (already) LOW //  discharge the capacitor through 300Ω on D8
    LowPower.powerDown(SLEEP_15MS, ADC_OFF, BOD_OFF);
...
// ============================================================================================================
// ============================================================================================================
#endif //end of #if defined(readD9resistorwD6ref_2byte) || defined(readD7resistorwD6ref_2byte)
// ============================================================================================================
// ============================================================================================================
...
void turnOnGreenAndBlueLED(){               // HIGHEST VISIBILITY 'blue-green' pip ~100uA
      ...
      bitClear(PORTB,4); bitSet(DDRB,4);    // D12 Common GND   // same as digitalWrite(12,LOW); pinMode(12,OUTPUT);
    #elif defined(LED_GndGB_A0_A2)
...
void turnOnRedAndBlueLED(){
      ...
      bitClear(PORTB,4); bitSet(DDRB,4);    // D12 Common GND   // same as digitalWrite(12,LOW); pinMode(12,OUTPUT);
    #elif defined(LED_GndGB_A0_A2)          // this configuration does not connect the RED channel of the 5mm LED!
...
void turnOnRedLED(){
    #if defined(LED_r9_b10_g11_gnd12)
      DDRB &= B11110001;                    // how set D9-D11 pins to INPUT with one command
...
void turnOffAllindicatorLEDs(){
    #if defined(LED_r9_b10_g11_gnd12)
      bitClear(PORTB,1); bitClear(DDRB,1);  // D9  [Red]    OFF  // same as pinMode(9,INPUT); digitalWrite(9,LOW);
      bitClear(PORTB,2); bitClear(DDRB,2);  // D10 [Blue]   OFF  // same as pinMode(10,INPUT);digitalWrite(10,LOW);
      bitClear(PORTB,3); bitClear(DDRB,3);  // D11 [Green]  OFF  // same as pinMode(11,INPUT);digitalWrite(11,LOW);
    #elif defined(LED_GndGB_A0_A2)
      bitClear(PORTC,1); bitClear(DDRC,1);  // A1 [green] OFF // same as pinMode(A1,INPUT); digitalWrite(A1,LOW);
